...
현재 서비스 검색은 답변 가능 11문항에서 `Recall@5 0.9091`, `Complete@5 0.9091`, `MRR@10 0.7273`이다. q006은 필요한 두 학교급 근거를 top-5에 함께 모으지 못한다. q008은 적용 조건과 내용 검색문을 분리해 목록 근거를 1위로 올리고 시행일 문단도 함께 보존했다. 변경 전 기준선은 `experiments/regression_retrieval_baseline.json`, 현재 생성 포함 결과는 `experiments/regression_generation_scope_query.json`에 보존했다.
...
이후에는 같은 답을 뒷받침하는 여러 판본을 하나의 근거 그룹으로 묶어 평가 규칙을 다시 정리했다. 원문 질문 Dense의 Complete@5는 `9/11`이다. 모든 질문을 손으로 줄이는 방식은 한 질문을 회귀시켜 적용하지 않았다. 대신 날짜·고시명이 포함된 내용 질문만 자동으로 분리하는 좁은 규칙을 적용했고, 현재 개발셋 Complete@5는 `10/11`이다.
