...
| 검색 | MRR@10 | 0.8939 |
| 생성 | 답변 가능 질문 성공률 | 1.0000 |
| 생성 | 답변 불가 질문 거절률 | 1.0000 |
| 생성 | 출처 형식 검증 통과율 | 1.0000 |
...
q011은 정답 표를 1위로 찾고도 2년 기준 값을 연간 값으로 표현하던 질문이었다. 질문에 나온 행과 학년 열을 표에 연결해 `국어 · 3~4학년 · 408시간 · 2년간`으로 구조화하고, 표 셀과 기간 주석을 함께 인용하도록 바꿨다. 현재 11개 답변 가능 질문은 모두 생성과 출처 검사를 통과한다.

전체 결과는 [검색·생성 회귀 결과](experiments/regression_generation_table_context.json), 질문 구조별 차이는 [실패 유형 분석](notes/2026-09-11-failure-type-analysis.md)에 남겼다. q011을 고친 과정과 제한 사항은 [표 기간 해석 실험](notes/2026-09-11-structured-table-context.md)에 따로 정리했다.
...
| 질문 기반 표 구조화 | q011의 408시간을 2년간 값으로 보존 | 명시적인 학년군 표 질문에 적용 |
...
현재 Python 테스트는 151개, 화면 JavaScript 테스트는 12개다. GitHub Actions에서는 Python·JavaScript·Spring 테스트와 CPU용 Docker 이미지 빌드를 확인한다.
...
- 다른 병합 셀 표에서도 행·열·기간 구조화가 동작하는지 검수하기
